 ...
 *   - 5 flash wait states (vs 2 on F103)
...
static void set_flash_latency(uint32_t sysclk_hz)
{
    ...
    if      (sysclk_hz <= 30000000U)  ws = FLASH_ACR_LATENCY_0WS;
    else if (sysclk_hz <= 60000000U)  ws = FLASH_ACR_LATENCY_1WS;
    else if (sysclk_hz <= 90000000U)  ws = FLASH_ACR_LATENCY_2WS;
    else if (sysclk_hz <= 120000000U) ws = FLASH_ACR_LATENCY_3WS;
    else if (sysclk_hz <= 150000000U) ws = FLASH_ACR_LATENCY_4WS;
    else                              ws = FLASH_ACR_LATENCY_5WS;
...
{
    uint32_t input = (src == RCC_PLLSRC_HSE) ? RCC_HSE_FREQ_HZ : RCC_HSI_FREQ_HZ;
    uint32_t vco = (input / pll->pllm) * pll->plln;
    return vco / pll->pllp;
}

// --- 180 MHz config: HSE 8MHz, PLL M=4 N=180 P=2 (tested on Nucleo) ---
...
void RCC_SystemClock_Config_HSI(void)
{
    ...
    FLASH->ACR = FLASH_ACR_LATENCY_0WS | FLASH_ACR_ICEN
...
RCC_Status_t RCC_Init(const RCC_ClkConfig_t *cfg)
{
    ...
    // switch sysclk
    {
        ...
        default:             sw = 0;                sws = RCC_CFGR_SWS_HSI; break;
        }
...
// MCO1 on PA8 — F4 GPIO setup is different from F1 (MODER/AFR instead of CRL/CRH)
// F4 MCO has a prescaler, F1 only had PLL/2 hardwired
