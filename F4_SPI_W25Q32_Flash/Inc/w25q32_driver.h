/*
 * w25q32_driver.h — Winbond W25Q32 SPI NOR flash driver (F446 version)
 *
 * 32 Mbit (4 MB) SPI NOR flash, 24-bit addressing.
 * Sits on top of stm32f4_spi_driver — bus is SPI1, CS is PA4 (manual GPIO).
 *
 * The flash chip itself doesn't care which MCU is talking to it. The
 * SPI protocol is identical to the F1 version of this driver. The only
 * thing that differs between F1 and F4 here is the CS GPIO setup.
 *
